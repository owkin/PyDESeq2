...
def test_deseq(tol=0.02):
    ...
    r_res = pd.read_csv(os.path.join(test_path, "data/r_test_res.csv"), index_col=0)
...
def test_lfc_shrinkage(tol=0.02):
    ...
    r_shrunk_res = pd.read_csv(
        os.path.join(test_path, "data/r_test_lfc_shrink_res.csv"), index_col=0
    )
    ...
    r_size_factors = pd.read_csv(
        os.path.join(test_path, "data/r_test_size_factors.csv"), index_col=0
    ).squeeze()

    r_dispersions = pd.read_csv(
        os.path.join(test_path, "data/r_test_dispersions.csv"), index_col=0
    ).squeeze()
    ...
    shrunk_res = res.lfc_shrink()

    # Check that the same LFC and lfcSE are found (up to tol)
    assert (
    ...
    assert (
        abs(r_shrunk_res.lfcSE - shrunk_res.lfcSE) / abs(r_shrunk_res.lfcSE)
    ).max() < tol
